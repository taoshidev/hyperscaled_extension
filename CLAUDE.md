# Beanstock Trading Extension — Claude Guidelines
...
A Chrome extension (MV3) for active traders to monitor their Beanstock Trading funded trading account, open positions on Hyperliquid, and challenge progress in real-time. Built with HTML/CSS/Vanilla JS. No build step.
...
Active/professional traders using Beanstock Trading's funded challenge program. They open this extension mid-session to check P&L, challenge progress, and drawdown — fast glances under pressure. They are technically literate, data-hungry, and performance-oriented. The job to be done: get accurate status information instantly without disrupting their trading flow.

### Brand Personality

**Precise · Powerful · Elite**

Beanstock Trading is a performance tool for serious traders. The tone is confident and authoritative — never playful or casual. Think Bloomberg Terminal meets modern web design. Every element earns its place.
...
- **Accent:** Beanstock green (#3edd5c) used sparingly for positive/interactive states. Amber (#ffb900) for caution. Red for danger
...
3. **Hierarchy through restraint.** Depth and emphasis come from opacity, not color. Use white at 2–8% to layer cards. Reserve green for exactly one thing per view.
...
Standard readable contrast is sufficient. No specific WCAG target. Ensure green on dark backgrounds meets basic legibility.
